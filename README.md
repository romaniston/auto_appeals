...
Структура:
- auto_appeals.py 	&nbsp- скрипт для автоматизации заполнения форм.
- funcs.py 		      &nbsp- модуль с функциями, классом и методами, которые использует auto_appeals.py
- links.py 		      &nbsp- модуль со ссылками в словаре на веб страницу и веб элементы
- done_delete.py 	  &nbsp- скрипт для очищения статусов задач в DoneDelete.xlsx
- Appeals.xlsx 	    &nbsp- таблица для заготовок задач для последующего заведения в формы.
- DoneDelete.xlsx 	&nbsp- таблица статусов обращений
____________________________________________________________
Инструкция:
